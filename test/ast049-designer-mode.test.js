...
test('AST-049 files are loaded and version/cache stay on PR #216 values', () => {
  assert.match(index(), /css\/designer-mode\.css\?v=ast057/);
  assert.match(index(), /js\/designer-mode\.js\?v=ast057/);
  assert.match(css(), /\.designer-panel/);
  assert.match(app(), /const APP_VERSION='3\.6\.54'/);
  assert.match(fs.readFileSync('service-worker.js', 'utf8'), /const CACHE_NAME='budde-3-6-54'/);
});
