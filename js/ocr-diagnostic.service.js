(function(){
  ...
  function recordTime(item){const value=item?.updatedAt||item?.createdAt||'';const time=Date.parse(value);return Number.isFinite(time)?time:0}
  function mergeById(localItems=[],incomingItems=[]){const merged=new Map();[...localItems,...incomingItems].forEach(item=>{if(!item?.id)return;const current=merged.get(item.id);if(!current||recordTime(item)>=recordTime(current)){const next={...(current||{}),...item};if(!next.imageData&&current?.imageData)next.imageData=current.imageData;merged.set(item.id,next)}});return [...merged.values()].sort((a,b)=>recordTime(a)-recordTime(b))}
  ...
  function linkTicket(ticketId,expenseId){const tickets=loadTickets();const ticket=tickets.find(item=>item.id===ticketId);if(ticket&&!ticket.linkedExpenseId){ticket.linkedExpenseId=expenseId;ticket.updatedAt=now();saveTickets(tickets)}return ticket||null}
  ...
  function exportSnapshot(){const current=purge();return {schemaVersion:1,exportedAt:now(),retentionMode:getRetentionMode(),tickets:current.tickets,diagnostics:current.diagnostics}}
  function mergeSnapshot(snapshot={}){const tickets=mergeById(loadTickets(),Array.isArray(snapshot.tickets)?snapshot.tickets:[]);const diagnostics=mergeById(loadDiagnostics(),Array.isArray(snapshot.diagnostics)?snapshot.diagnostics:[]);saveTickets(tickets);saveDiagnostics(diagnostics);if(['months3','months6','months12','manual'].includes(snapshot.retentionMode))localStorage.setItem(RETENTION_KEY,snapshot.retentionMode);purge();return {tickets:tickets.length,diagnostics:diagnostics.length}}
  ...
  window.TicketArchiveService={create:createTicket,get:getTicket,link:linkTicket,stats:ticketStats,clear:clearTickets,purge,getRetentionMode,setRetentionMode,exportSnapshot,mergeSnapshot};
  window.OcrDiagnosticService={create,stats,clear,clearTickets,clearAll,exportZip,purge,getRetentionMode,setRetentionMode,exportSnapshot,mergeSnapshot};
})();