(function(){
  ...
  function render(stateName='neutral',options={}){
    ...
    const avatar=states[state].avatar
      ...
      : 'B';
    target.innerHTML=`<section class="buddy buddy--${state}" aria-label="Buddy"><div class="buddyAvatar" data-state="${state}" aria-hidden="true">${avatar}</div><div class="buddyBubble"><p>${message}</p></div>${trustGauge(options.trust)}</section>`;
  }
