...
async function backupToGoogleDrive(){const backupButton=document.getElementById('googleDriveBackupButton'),backupStatus=document.getElementById('googleDriveBackupStatus');if(!window.GoogleDriveAdapter){if(backupStatus)backupStatus.textContent='Sauvegarde Google Drive indisponible.';return}try{if(backupButton)backupButton.disabled=true;if(backupStatus)backupStatus.textContent='Sauvegarde Google Drive en cours…';const result=await GoogleDriveAdapter.save(db);lastGoogleDriveBackupAt=new Date();if(backupStatus)backupStatus.textContent=`Dernière sauvegarde : ${formatDateTime(lastGoogleDriveBackupAt)}`;showToast('Sauvegarde Google Drive terminée');console.info('Sauvegarde Google Drive Budd€',result)}catch(error){console.warn('Sauvegarde Google Drive impossible.',error);if(backupStatus)backupStatus.textContent=error?.message||'Erreur pendant la sauvegarde Google Drive.';showToast('Erreur sauvegarde Drive')}finally{renderGoogleAuthStatus()}}
...
if(window.GoogleAuthService){GoogleAuthService.onChange(renderGoogleAuthStatus);GoogleAuthService.init().catch(error=>{console.warn('Initialisation Google Auth impossible.',error);renderGoogleAuthStatus()}).then(renderGoogleAuthStatus);googleAuthButton.onclick=async()=>{try{if(GoogleAuthService.isSignedIn())GoogleAuthService.signOut();else await GoogleAuthService.signIn()}catch(error){console.warn('Connexion Google interrompue.',error)}renderGoogleAuthStatus()};if(window.googleDriveBackupButton)googleDriveBackupButton.onclick=backupToGoogleDrive}
