...
function initGoogleDriveBlock(){const authButton=document.getElementById('googleAuthButton'),backupButton=document.getElementById('googleDriveBackupButton'),restoreButton=document.getElementById('googleDriveRestoreButton'),backupStatus=document.getElementById('googleDriveBackupStatus'),backupButtons=document.querySelectorAll('#googleDriveBackupButton');if(backupButtons.length!==1){const message=backupButtons.length?'Erreur Google Drive : plusieurs boutons de sauvegarde détectés.':'Erreur Google Drive : bouton de sauvegarde introuvable.';console.error(message,{count:backupButtons.length});if(backupStatus)backupStatus.textContent=message;return}if(backupButton)backupButton.addEventListener('click',backupToGoogleDrive);if(restoreButton)restoreButton.addEventListener('click',restoreFromGoogleDrive);if(!window.GoogleAuthService){renderGoogleAuthStatus();return}GoogleAuthService.onChange(()=>{renderGoogleAuthStatus();checkGoogleDriveStartupSync()});GoogleAuthService.init().catch(error=>{console.warn('Initialisation Google Auth impossible.',error);renderGoogleAuthStatus()}).then(()=>{renderGoogleAuthStatus();checkGoogleDriveStartupSync()});if(authButton){authButton.addEventListener('click',async()=>{try{if(GoogleAuthService.isSignedIn())GoogleAuthService.signOut();else await GoogleAuthService.signIn()}catch(error){console.warn('Connexion Google interrompue.',error)}renderGoogleAuthStatus();checkGoogleDriveStartupSync()})}}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initGoogleDriveBlock,{once:true});else initGoogleDriveBlock()
