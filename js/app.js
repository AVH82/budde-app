...
if(window.GoogleAuthService){GoogleAuthService.onChange(renderGoogleAuthStatus);GoogleAuthService.init().catch(error=>{console.warn('Initialisation Google Auth impossible.',error);renderGoogleAuthStatus()}).then(renderGoogleAuthStatus);googleAuthButton.onclick=async()=>{try{if(GoogleAuthService.isSignedIn())GoogleAuthService.signOut();else await GoogleAuthService.signIn()}catch(error){console.warn('Connexion Google interrompue.',error)}renderGoogleAuthStatus()}}
